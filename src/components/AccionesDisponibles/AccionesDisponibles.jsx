...
function AccionesDisponibles() {
    ...
    const myfields = (name, amount, price) => {
    ...
    const getInfo = async () => {
        try {
            ...
            const url = `${config.route}purchase/admin_ventas`; 
            console.log(url);
          ...
          <div className="MainDivListFields">
            {fields_shown.map(r => (
              <div key={r.id} className="labelfield">
                <p className="labelspecific">Usuario Administrador:</p>
                <p className="labelspecific">Empresa: {r.name}</p>
                <p className="labelspecific">Precio: {r.price}</p>
                <p className="labelspecific">Stock: {r.amount}</p>
                <button className='botonsubmit2' onClick={()=>myfields(r.name, r.amount, r.price)}>Comprar Acciones</button>
              </div>
            ))}
          </div>
        </div>
